    ...
    newBill.save((err, result) => {
        if (err) {
            ...
            if(req.body.payment_mode === 'Cash') {
                ...
                sessionModel.findOne({'session_status': 'true','branch': req.body.branch})
                .select('-__v -_id')
                .lean()
                .exec((err, result) => {
                    if (err) {
            ...
            totalModel.find({ 'date': time.getNormalTime(),'payment_mode': req.body.payment_mode, 'bill_by': req.body.user_name, 'branch': req.body.branch})
                .exec((err, totalS) => {
                    if (err) {
                        console.log(err)
