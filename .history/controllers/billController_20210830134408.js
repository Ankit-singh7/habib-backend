...
let createBill = (req, res) => {
    ...
    newBill.save((err, result) => {
        if (err) {
            ...
            totalModel.find({ 'date': time.getNormalTime(),'payment_mode': req.body.payment_mode, 'bill_by': req.body.user_name, 'branch_id': req.body.branch_id, 'employee_id':req.body.employee_id})
                .exec((err, totalS) => {
                    if (err) {
                        console.log(err)
        ...
        return new Promise((resolve,reject) => {
            if(bill.products.length>0) {
                ...
                for(let item of bill.products) {
                    ...
                    productSalesReportModel.findOne({'date': moment(bill.createdOn).format('DD-MM-YYYY'),'product_id': item.product_id,'branch_id':bill.branch_id}).exec((err,result) => {
                        if(err){
                            console.log(err)
                        } else if (check.isEmpty(result)) {
...
let updateDrawerBalance = () => {
        return new Promise((resolve,reject) => {
            if(bill.payment_mode === 'Cash') {
                ...
                sessionModel.findOne({'session_status': 'true','branch_id':bill.branch.branch_id})
                   .select('-__v -_id')
                   .lean()
                   .exec((err, sResult) => {
                     if (err) {
