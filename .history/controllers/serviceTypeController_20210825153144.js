...
let getSingleService = (req, res) => {
    serviceTypeModel.findOne({ 'service_id': req.params.id })
        .select('-__v -_id')
        .lean()
        .exec((err, result) => {
            if (err) {
...
let deleteService = (req,res) => {
    serviceTypeModel.findOneAndRemove({'service_id':req.params.id})
    .exec((err,result) => {
        if (err) {
            console.log(err)
...
let updateService = (req,res) => {
    let option = req.body
    serviceTypeModel.updateOne({'service_id':req.params.id},option,{multi:true})
    .exec((err,result) => {
        if (err) {
            console.log(err)
