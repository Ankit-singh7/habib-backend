...
let deleteAppointment = (req,res) => {
    branchModel.findOneAndRemove({'appointment_id':req.params.id})
    .exec((err,result) => {
        if (err) {
            console.log(err)
...
let updateAppointment = (req,res) => {
    let option = req.body
    branchModel.updateOne({'appointment_id':req.params.id},option,{multi:true})
    .exec((err,result) => {
        if (err) {
            console.log(err)
...
let getBillingCustomerAppointment = (req,res) => {
    appointmentModel.findOne({ 'customer_name': (req.params.customer_name).toLowerCase(), 'status': 'pending', 'appointment_date': time.getNormalTime(), 'branch_id': req.params.branch_id }).exec((err, result) => {
        if (err) {
            console.log(err);
        } else if (check.isEmpty(result)) {
            let apiResponse = response.generate(true, 'No Detail Found', 404, null)
